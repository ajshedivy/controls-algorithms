...
class Policy(object):
    ...
    def _placeholders(self):
        ...
        # learning rate:
        self.lr_ph = tf.compat.v1.placeholder(tf.float32, (), 'lr')
    ...
    def _policy_nn(self):
        ...
        out1 = tf.compat.v1.layers.dense(self.obs_ph, hid1_size, tf.tanh,
                              kernel_initializer=tf.compat.v1.random_normal_initializer(
                                  stddev=np.sqrt(1 / self.obs_dim)), name="h1")
        out2 = tf.compat.v1.layers.dense(out1, hid2_size, tf.tanh,
                              kernel_initializer=tf.compat.v1.random_normal_initializer(
                                  stddev=np.sqrt(1 / hid1_size)), name="h2")
        out3 = tf.compat.v1.layers.dense(out2, hid3_size, tf.tanh,
                              kernel_initializer=tf.compat.v1.random_normal_initializer(
                                  stddev=np.sqrt(1 / hid2_size)), name="h3")
        self.means = tf.compat.v1.layers.dense(out3
                                     , self.act_dim,
                                     kernel_initializer=tf.compat.v1.random_normal_initializer(
        ...
        self.log_vars = tf.reduce_sum(input_tensor=log_vars, axis=0) - 1.0
    ...
    def _sample(self):
        ...
        self.sampled_act = tf.add(self.means ,
                            ...
                            tf.exp(self.log_vars / 2.0) *tf.random.normal(shape=(self.act_dim,)),
                             name = 'sampledact')
    ...
    def _loss_train_op(self):
        ...
        """
        loss1 = -tf.reduce_mean(input_tensor=self.advantages_ph *                   # loss1 = E [(PI/PIold)*A]
    ...
    def _restore_model(self):
           ...
           loader = tf.compat.v1.train.import_meta_graph("./savedmodel/"+self.env_name+"/Policy/trained_variables.ckpt.meta")
